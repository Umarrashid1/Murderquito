...
class ServoCalibrator:
    ...
    def __init__(self, cam = Camera, servo_c = Servo_controller) -> None:
        self.cam_to_tilt_dist = 4.3
        self.cam_to_pan_dist = 4.6
    ...
    def center_laser(self, frame, det, servo_c, dist_to_wall = None):
        center = self.center
            ...
            if(las_coords[1] != center[1]):
                if(las_coords[1] < center[1] and self.tilt_angle < 180):
                    ...
                    if(las_coords[1] > center[1] + 20 and self.tilt_angle < 180):
                        ...
                        servo_c.tilt_servo.move(self.tilt_angle)
                        time.sleep(0.5)            
            if(las_coords[1] < center[1] + 5 and las_coords[1] > center[1] - 5):
                if(las_coords[0] < center[0] + 5 and las_coords[0] > center[0] - 5):
                    return True
            else: return False
            
            

    def find_indiv_center_angles(self,frame, det, servo_c, chosen_servo = None):
    ...
    def calc_dist_from_centerangle(self, servo_c = Servo_controller, choice = None):
        ...
        # b = a × tan(β)
        cam_to_wall_x = math.tan(tilt_angle) * self.cam_to_tilt_dist 
        cam_to_wall_y = math.tan(pan_angle) * self.cam_to_pan_dist 
        print(cam_to_wall_x)
        print(cam_to_wall_y)
